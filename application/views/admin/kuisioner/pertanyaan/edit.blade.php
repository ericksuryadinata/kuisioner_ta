    ...
    <div class="container-fluid">
        ...
        <div class="row clearfix">
            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                <div class="card">
                    <?php echo form_open_multipart(route('admin.kuisioner.pertanyaan.update'),'id="form_validation" novalidate="novalidate"')?>
                        <div class="header">
                            <h2>EDIT PERTANYAAN KUISIONER</h2>
                        </div>
                        <div class="body">
                            <label for="email_address">Pertanyaan Kuisioner</label>
                            <div class="form-group">
                                <div class="form-line">
                                    <textarea name="nama_pertanyaan" id="nama_pertanyaan" required="" aria-required="true">
                                        {{isset($pertanyaan_kuisioner->pertanyaan) ? $pertanyaan_kuisioner->pertanyaan : ''}}
                                    </textarea>
                                </div>
                            </div>
                            ...
                            <div class="form-group">
                                <select class="form-control show-tick" name="id_aspek" required="" aria-required="true" >
                                    ...
                                    @if (isset($pertanyaan_kuisioner->id_aspek))
                                        @foreach ($aspek as $key => $value)
                                            @if ($pertanyaan_kuisioner->id_aspek == $value->id)
                                                <option value="{{$value->id}}" selected>{{$value->nama_aspek}}</option>
                                            @else
                            ...
                            <input type="text" class="hidden" name="id" value="{{isset($pertanyaan_kuisioner->id) ? $pertanyaan_kuisioner->id : ''}}">
